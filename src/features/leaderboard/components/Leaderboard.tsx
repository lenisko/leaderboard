...
import type { ColDef, GridReadyEvent, ValueFormatterFunc, ValueGetterFunc, GridApi } from 'ag-grid-community';
...
const autoSizeColumns = (api: GridApi): void => {
  if (typeof window === 'undefined') {
    return api.sizeColumnsToFit();
  }

  const numberOfColumns = api.getColumns()?.length ?? 1;
    ...
    <>
      <div className="ag-theme-leaderboard h-[606px] w-full">
        <AgGridReact
          ...
          onGridReady={({ api }: GridReadyEvent) => autoSizeColumns(api)}
